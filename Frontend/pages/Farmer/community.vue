<template>
  <div class="min-h-screen bg-gray-50 font-poppins">
    ...
    <div class="community-page py-8 px-4 space-y-8 w-full flex flex-col md:flex-row bg-white mt-16">
            ...
            <transition name="fade">
                <div v-if="activeFarmers.length">
                    ...
                    <div v-for="farmer in activeFarmers" :key="farmer.id" class="flex items-center mb-4">
                        ...
                        <p class="text-sm text-gray-600">{{ farmer.first_name }} {{ farmer.last_name }} ({{ farmer.phone
                            }})</p>
                    </div>
                </div>
        ...
        <div class="w-full md:w-3/4 pl-4">
            <UCard class="new-post mb-6 shadow-lg bg-white p-4 rounded-lg" :style="{ backgroundColor: 'white', }">
                <UInput type="textarea" v-model="newPost" placeholder="Share your thoughts..." rows="3"
                    class="resize-none rounded-lg border-gray-300 focus:border-green-500"
                    :style="{ backgroundColor: 'white', color: 'black' }" />
                <UButton block class="mt-3 text-white bg-green-500 hover:bg-green-600 transition-all"
                    @click="submitPost" :disabled="!newPost.trim()">
                    Post
                </UButton>
            </UCard>
                    ...
                    <div class="comments mt-4">
                        ...
                        <div v-if="post.comments?.length" class="space-y-2">
                            <div v-for="comment in post.comments" :key="comment.id" class="bg-gray-100 p-2 rounded-lg">
                                <strong class="text-green-700">{{ comment.author }}:</strong> <span
                                    class="text-black">{{ comment.content }}</span>
                            </div>
                        </div>
                        ...
                        <UInput type="textarea" v-model="newComments[post.id]" placeholder="Write a comment..." rows="2"
                            ...
                            :style="{ backgroundColor: 'white', color: 'black' }" />
                        <UButton block class="mt-2 text-white bg-green-500 hover:bg-green-600 transition-all"
                            @click="addComment(post.id)" :disabled="!newComments[post.id]?.trim()">
                            Comment
                        </UButton>
                    </div>
    ...
    <button
        class="md:hidden fixed bottom-6 right-6 bg-green-500 text-white p-4 rounded-full shadow-lg hover:bg-green-600 transition-all"
        @click="toggleFarmers">
