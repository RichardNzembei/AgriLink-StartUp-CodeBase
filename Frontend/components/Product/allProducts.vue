    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      ...
      >
        ...
        <div class="p-3">
          ...
          <p class="text-gray-600 text-sm">Seller: <span class="text-blue-600 font-medium">{{ product.ownerPhone }}</span></p>
        </div>

        <!-- View More Button -->
        <button @click="toggleDetails(product.id)" class="mt-2 bg-gray-200 w-full py-2 rounded hover:bg-gray-300 transition flex items-center justify-center">
        ...
        <div v-if="expandedProducts.includes(product.id)" class="mt-4">
          ...
          <div class="flex justify-between items-center">
            <a :href="'tel:' + formatPhone(product.ownerPhone)" class="bg-blue-500 text-white p-2 rounded-full shadow hover:bg-blue-600 transition">
              <i class="fas fa-phone-alt"></i>
            </a>
            <a :href="'https://wa.me/' + formatWhatsApp(product.ownerPhone)" target="_blank" class="bg-green-500 text-white p-2 rounded-full shadow hover:bg-green-600 transition">
...
/* Ensure products fit well on small screens */
