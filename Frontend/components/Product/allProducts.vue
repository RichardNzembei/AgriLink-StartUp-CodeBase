    ...
    >
      ...
      <div class="space-y-4">
        <div class="flex items-center text-sm text-gray-700">
          <UIcon name="i-heroicons-currency-dollar" class="w-4 h-4 mr-2 text-green-600" />
          <span>Price: <span class="font-semibold">{{ selectedProduct.price }}</span> per {{ selectedProduct.unit }}</span>
        </div>
        <div class="flex items-center text-sm text-gray-700">
          <UIcon name="i-heroicons-scale" class="w-4 h-4 mr-2 text-green-600" />
          <span>Supply: <span class="font-semibold">{{ selectedProduct.supplyAmount }}</span> {{ selectedProduct.unit }}s</span>
        </div>
        <div class="flex items-center text-sm text-gray-700">
          <UIcon name="i-heroicons-user-circle" class="w-4 h-4 mr-2 text-green-600" />
        ...
        <div class="flex justify-between items-center mt-6">
          <a
            :href="'tel:' + formatPhone(selectedProduct.ownerPhone)"
          ...
          <a
            :href="'https://wa.me/' + formatWhatsApp(selectedProduct.ownerPhone)"
            target="_blank"
        ...
        <div class="mt-6 border-t pt-4">
          ...
          <div v-if="(selectedProduct.reviews ?? []).length > 0" class="space-y-3">
            <div
              v-for="(review, index) in (selectedProduct.reviews ?? []).slice(0, 2)"
              :key="index"
          ...
          <input
            v-model="newProductReviews[selectedProduct.id]"
            type="text"
          ...
          />
          <button
            @click="addProductReview(selectedProduct.id)"
        ...
        <div class="mt-6 border-t pt-4">
          ...
          <div v-if="(selectedProduct.sellerReviews ?? []).length > 0" class="space-y-3">
            <div
              v-for="(review, index) in (selectedProduct.sellerReviews ?? []).slice(0, 2)"
              :key="index"
              class="bg-gray-50 p-3 rounded-lg"
            >
              <p class="text-sm text-gray-700">
          ...
          <p v-else class="text-gray-500 text-sm">No reviews yet.</p>
          <input
            v-model="newSellerReviews[selectedProduct.ownerPhone]"
            type="text"
            placeholder="Write a review..."
            class="w-full p-2 mt-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all"
          />
          <button
            @click="addSellerReview(selectedProduct.ownerPhone)"
            class="w-full mt-2 bg-green-500 text-white py-2 rounded-lg hover:bg-green-600 transition-colors duration-300"
          >
